...
function buildRecommendations(platform, env, shell) {
  ...
  if (platform === "win32") {
    ...
    recs.push("Set LANG=en_US.UTF-8 and LC_ALL=en_US.UTF-8 in your shell profile (.bashrc / .zshrc).");
    recs.push("Use a terminal with Arabic shaping: iTerm2, GNOME Terminal, Kitty, or Alacritty.");
  }

  recs.push("Run `arabic-cli-fix run <command>` to start tools with UTF-8 env vars applied.");
  recs.push("Run `arabic-cli-fix test` to visually verify Arabic rendering in your terminal.");
  recs.push("Run `arabic-cli-fix init-ai` to configure AI tools for Arabic-safe output.");
  recs.push("For Arabic output, write to a UTF-8 Markdown file and review in VS Code or a browser.");
